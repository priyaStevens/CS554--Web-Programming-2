...
const ComicsList = (props) => {
    ...
    // const baseUrl = 'https://gateway.marvel.com:443/v1/public/comics?limit=100';
...
let { ComicListData} = useAxios(baseUrl);
...
const searchValue = async (value) => {
	...
	if (loading) {
		...
		<br/>
		<div>
			{!searchTerm && currentPage > 0 ?(<Link className='showlink' to={`/comics/page/`+(parseInt(currentPage)-1)}><Button className={classes.buttonclass} variant="contained" color='secondary' size='medium'>Previous</Button></Link>) : ''}

			{!searchTerm && comicsData && comicsData.length >= 100 ?(<Link className='showlink' to={`/comics/page/`+(parseInt(currentPage)+1)}><Button className={classes.buttonclass} variant="contained" color='secondary'
			 size='medium'>Next</Button></Link>) : ''}

			{searchData && searchData.length >= 100 ?(<Link className='showlink' to={`/comics/page/`+(parseInt(currentPage)+1)}><Button className={classes.buttonclass} variant="contained" color='secondary' size='medium'>Next</Button></Link>) : ''}
